...
// Only allow 'light' and 'dark' theme modes
...
function readDomState(): Partial<PreferencesState> {
  ...
  // Only allow 'light' and 'dark' theme modes
  let themeModeAttr = getSafeValue(root.getAttribute("data-theme-mode"), THEME_MODE_VALUES);
  const resolvedMode = root.classList.contains("dark") ? "dark" : "light";
  if (!themeModeAttr || (themeModeAttr as string) === "system") themeModeAttr = resolvedMode;
...
export const PreferencesStoreProvider = ({
  ...
  useEffect(() => {
    ...
    const applyFromMode = (mode: PreferencesState["themeMode"]) => {
      unsubscribeMedia?.();
      // If mode is 'system', fallback to 'light' (or you can choose 'dark')
      const safeMode = mode === "system" ? "light" : mode;
      const resolved = applyThemeMode(safeMode);
